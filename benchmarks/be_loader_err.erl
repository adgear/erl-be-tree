      ...
      case term_reader:start_link(FileName) of
        ...
        {ok, PidReader} ->
          case term_reader:read(PidReader) of
            ...
            {ok, Term} ->
              Domains = [Term],
              case erl_betree:betree_make_err(Domains) of
                {ok, Betree} ->
                  Consts = [],
                  Index = 0,
...
add_expr(Expr, #be_evaluator{
  ...
  Index1 = Index + 1,
  case erl_betree:betree_make_sub_err(Betree, Index1, Consts, Expr) of
    {ok, Sub} ->
      case erl_betree:betree_insert_sub_err(Betree, Sub) of
        ok ->
          {ok, State#be_evaluator{index = Index1}};
        X ->
          {{error, {betree_insert_sub_err, X}}, State}
      end;
    X ->
      {{error, {betree_make_sub_err, X}}, State}
  end.
