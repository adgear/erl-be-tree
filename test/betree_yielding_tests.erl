...
check_yielding(Betree, Event, undefined) ->
  ...
  ?assertEqual(ok, R_no_yielding),
  {R_yielding, _} = erl_betree:betree_search_yielding(Betree, Event, 100),
  ?assertEqual(ok_continuation, R_yielding);
check_yielding(Betree, Event, Expect) ->
  R_no_yielding = erl_betree:betree_search_yielding(Betree, Event, 0),
  ?assertEqual({ok, Expect}, R_no_yielding),
  R_yielding = erl_betree:betree_search_yielding(Betree, Event, 100),
  ?assertEqual({ok_continuation, Expect}, R_yielding).

-record(basic_first_test, { i }).
first_test() ->
  Domains = [[{i, int, disallow_undefined}]],
  {ok, Betree} = erl_betree:betree_make(Domains),
  Event = [#basic_first_test{ i = 5 }],
  check_yielding(Betree, Event, []).
...
freq_test() ->
  ...
  ?assertError(badarg, erl_betree:betree_search_yielding(Betree, Event, 0)),
  ?assertError(badarg, erl_betree:betree_search_yielding(Betree, Event, 100)).
...
bug_geo_test() ->
  ...
  check_yielding(Betree, Event, [1,2]).
...
multiple_trees_test() ->
  ...
  check_yielding(Betree1, Event1, [1]),
  check_yielding(Betree1, Event2, [ ]),
  check_yielding(Betree2, Event1, [ ]),
  check_yielding(Betree2, Event2, [2]),
  check_yielding(Betree1, Event1, [1]),
  check_yielding(Betree1, Event2, [ ]).
...
list_bug_test() ->
  ...
  lists:foreach(fun ({Id, Expr}) ->
    ...
    ok = erl_betree:betree_insert_sub(Betree, Sub),
    ok end, Exprs),
  check_yielding(Betree, Event, Ids).
...
exists_test() ->
  ...
  lists:foreach(fun ({Id, Expr}) ->
    ...
    ok end, Exprs),
  T1 = os:timestamp(),
  check_yielding(Betree, GoodEvent, undefined),
  T2 = os:timestamp(),
  ...
  T5 = os:timestamp(),
  check_yielding(Betree, BadEvent, []),
  T6 = os:timestamp(),
...
sub_test() ->
  ...
  ok = erl_betree:betree_insert_sub(Betree, Sub),
  Event = [#sub{i = 0}],
  check_yielding(Betree, Event, [Id]).
...
unsorted_test() ->
  ...
  ok = erl_betree:betree_insert_sub(Betree, Sub),
  check_yielding(Betree, [#unsorted{il = [9, 10, 77, 29, 29, 84, 2]}], [Id]).
...
run_valid(Domain, AllowUndefinedBool, Expr, Validity) ->
  ...
  case Validity of
    ...
    {valid, MatchVar, NoMatchVar} ->
      ...
      check_yielding(Betree, MatchValue, [Id]),
      NoMatchValue =
        case NoMatchVar of
          null -> [];
          _ -> [#valid{var = NoMatchVar}]
        end,
      check_yielding(Betree, NoMatchValue, []),
      ok
  end.
